...
def video2image(videopath, imagepath, fps=0, start_time=0, last_time=0):
    if start_time == 0:
        if fps == 0:
            os.system('ffmpeg -hwaccel cuda -i "'+videopath +
                      '" -f image2 '+'-q:v -0 '+imagepath)
        else:
            os.system('ffmpeg -hwaccel cuda  -i "'+videopath+'" -r '+str(fps) +
                      ' -f image2 '+'-q:v -0 '+imagepath)
    else:
        if fps == 0:
            os.system('ffmpeg -hwaccel cuda  -ss '+start_time+' -t '+last_time +
                      ' -i "'+videopath+'" -f image2 '+'-q:v -0 '+imagepath)
        else:
            os.system('ffmpeg -hwaccel cuda  -ss '+start_time+' -t '+last_time+' -i "' +
                      videopath+'" -r '+str(fps)+' -f image2 '+'-q:v -0 '+imagepath)
...
def cut_video(in_path, start_time, last_time, out_path, vcodec='h265'):
    if vcodec == 'copy':
        ...
        os.system('ffmpeg -ss '+start_time+' -t '+last_time +
                  ' -i "'+in_path+'" -vcodec libx264 -b 12M '+out_path)
    elif vcodec == 'h265':
        os.system('ffmpeg -ss '+start_time+' -t '+last_time +
                  ' -i "'+in_path+'" -vcodec libx265 -b 12M '+out_path)
